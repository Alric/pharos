class InstitutionsController < ApplicationController
  ...
  def index
    respond_to do |format|
      ...
      @institutions = @institutions.where(receiving_bucket: params[:receiving_bucket]) if params[:receiving_bucket].present?

      @institutions = @institutions.order('name')
      @sizes = find_all_sizes unless request.url.include?('/api/')
      @count = @institutions.count
      page_results(@institutions)
      format.json { render json: { count: @count, next: @next, previous: @previous, results: @institutions.map(&:serializable_hash) } }
  ...
  def single_snapshot
    ...
    if @institution.is_a?(MemberInstitution)
      ...
      respond_to do |format|
        format.json { render json: { institution: @institution.name, snapshots: @snapshots.map(&:serializable_hash) } }
        format.html do
          redirect_to root_path
  ...
  def group_snapshot
    ...
    respond_to do |format|
      format.json { render json: { snapshots: @snapshots.each { |snap_set| snap_set.map(&:serializable_hash) } } }
      format.html do
        redirect_to root_path
  ...
  def deletion_notifications
    ...
    Institution.all.each do |current_inst|
      ...
      next if items.nil? || items.count == 0
  ...
  def build_bulk_deletion_list
    ...
    initial_identifiers.each do |identifier|
      current = IntellectualObject.find_by(identifier: identifier)
      current = GenericFile.find_by(identifier: identifier) if current.nil?
  ...
  def bulk_mark_deleted
    @bulk_job.intellectual_objects.each do |obj|
      next unless WorkItem.deletion_finished?(obj.identifier)
  ...
  def write_snapshots_to_spreadsheet
    ...
    while date_column == 0
      ...
      date_column = counter if cell.blank? || cell == ''
      counter += 1
    end
    i = 1
    unless date_column == 0
      ...
      while i < 2000
        ...
        unless cell.nil?
          if @wb_hash.key?(cell)
            cs_gb = (@wb_hash[cell][0].to_f / 1_073_741_824).round(2)
            go_gb = (@wb_hash[cell][1].to_f / 1_073_741_824).round(2)
