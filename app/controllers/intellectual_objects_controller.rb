class IntellectualObjectsController < ApplicationController
  ...
  def index
    ...
    @intellectual_objects = current_user.admin? && @institution.identifier == Pharos::Application::APTRUST_ID ? IntellectualObject.all : IntellectualObject.discoverable(current_user).with_institution(@institution.id)
    filter_count_and_sort
    page_results(@intellectual_objects)
    options_hash = params[:with_ingest_state] == 'true' && current_user.admin? ? { include: [:ingest_state] } : {}
  ...
  def show
    if @intellectual_object
      ...
      respond_to do |format|
        format.json { render json: { status: 'error', message: 'This object could not be found.', url: request.original_url }, status: :not_found }
  ...
  def finished_destroy
    ...
    if deletion_item.nil?
      respond_to do |format|
        format.json { render json: { status: 'error', message: 'There is no existing deletion request for the specified object.', intellectual_object_identifier: @intellectual_object.identifier }, status: :not_found }
        format.html { redirect_to root_url, alert: "An intellectual object with identifer: #{params[:intellectual_object_identifier]} could not be found." }
      end
      return
    end
    ...
    attributes[:outcome_information] = deletion_item.aptrust_approver.nil? || deletion_item.aptrust_approver == '' ? "Object deleted at the request of #{deletion_item.user}. Institutional Approver: #{deletion_item.inst_approver}." : "Object deleted as part of bulk deletion at the request of #{deletion_item.user}. Institutional Approver: #{deletion_item.inst_approver}. APTrust Approver: #{deletion_item.aptrust_approver}"
  ...
  def restore
    if @intellectual_object
      ...
      if @intellectual_object.state == 'D'
        ...
        restore_item = if @intellectual_object.storage_option == 'Standard'
                         WorkItem.create_restore_request(@intellectual_object.identifier, current_user.email)
                       else
                         WorkItem.create_glacier_restore_request(@intellectual_object.identifier, current_user.email)
                       end
  ...
  def object_as_json
    ...
    options_hash = current_user.admin? ? { include: include_list } : { include: include_list, except: [:bagit_profile_identifier] }
