  ...
  describe 'POST create' do
    ...
    describe 'when signed in' do
      ...
      it 'should be forbidden when the file belongs to your institution' do
        post :create, params: { generic_file_identifier: generic_file_one.identifier, checksum: { algorithm: 'md5', datetime: Time.zone.now, digest: '1234567890' } }, format: :json
        expect(response.status).to eq 403
      end

      it 'should be forbidden when the file does not belong to your institution' do
        post :create, params: { generic_file_identifier: generic_file_two.identifier, checksum: { algorithm: 'md5', datetime: Time.zone.now, digest: '1234567890' } }, format: :json
        expect(response.status).to eq 403
      end
    end
